...
class BehaviorDate(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        buy_inter = self.behavior_dict[self.behaviors[-1]].get(str(u_id), None) # 특정 user가 구매한 item이 하나도 없다면 signal = [0, 0, 0, 0]
    ...
    def __get_sparse_interact_dict(self):
        ...
        for behavior in self.behaviors:
            ...
            with open(os.path.join(self.path, behavior + '.txt'), encoding='utf-8') as f:
                ...
                self.inter_matrix.append(inter_matrix) # inter matrix for local behaviors
                all_row.extend(row)
                all_col.extend(col)
        ...
        self.all_inter_matrix = sp.coo_matrix((values, (all_row, all_col)), [self.user_count + 1, self.item_count + 1]) # inter matrix for global graph
        
        """
        Load Behavior-aware augmented graph
        """
        with open(os.path.join(self.path, 'aug_r.txt'), encoding='utf-8') as f:
            ...
            values = torch.ones(len(row), dtype=torch.float32)
            inter_matrix = sp.coo_matrix((values, (row, col)), [self.user_count + 1, self.item_count + 1])
            self.aug_inter_matrix = inter_matrix 
