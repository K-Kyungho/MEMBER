...
class Trainer(object):
    ...
    def train_model(self):
        ...
        for epoch in range(self.epochs):
            self.model.glo_graph_aug = LightGCN(self.model.device, self.model.layers, self.model.n_users + 1, self.model.n_items + 1, self.dataset.aug_inter_matrix, self.model.dropout) 
            #Random edge dropout for each epoch
            self.model.train()
