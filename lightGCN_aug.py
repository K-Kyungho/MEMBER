...
class LightGCN(nn.Module):
    ...
    def __init__(self, device, n_layers, n_users, n_items, interaction_matrix, aug_p):
        ...
        self.aug_p = aug_p
        self.A_index, self.A_data, self.A_size = self._get_index_data()
        #print(f"A_index: {self.A_index}, A_data: {self.A_data.shape}, A_size: {self.A_size}")
        self.A_adj_matrix = self._get_a_adj_matrix(self.A_index, self.A_data, self.A_size, self.aug_p)
    ...
    def _get_index_data(self):
        ...
        #row = np.array(A_adj.row)
        #col = np.array(A_adj.col)
        #index = torch.LongTensor(np.vstack((row, col)))
    ...
    def _get_a_adj_matrix(self, index, data, size, aug_p):
        #print(index)
        #print(data.shape)
        remain_idxs = list(np.random.choice(index.shape[1], size = int(index.shape[1]*(1-aug_p))))
        new_index = index[:, remain_idxs]
        new_data = data[remain_idxs]
        A_sparse = torch.sparse.FloatTensor(new_index, new_data, torch.Size(size))
        return A_sparse
